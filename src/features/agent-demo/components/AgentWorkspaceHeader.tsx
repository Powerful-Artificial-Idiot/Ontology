...
export function AgentWorkspaceHeader({ sessionId, turnCount, isRunning, onLoadExample, onReset }: { sessionId?: string; turnCount: number; isRunning: boolean; onLoadExample: () => void; onReset: () => void }) {
  return (
    <section className="flex h-[74px] shrink-0 items-center gap-5 border-b border-slate-200 bg-white px-5">
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2"><h1 className="truncate text-base font-bold text-slate-950">Traceable Agent Workspace</h1><span className="inline-flex items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-blue-700"><CircleDot className="h-2.5 w-2.5" />Scripted</span></div>
